...
func (r *StaffRepository) GetByID(ctx context.Context, id string) (*model.Staff, error) {
	s := &model.Staff{}
	query := `SELECT id, name, email, password_hash, role, phone, service_incentive_rate, product_incentive_rate, base_salary, monthly_target, is_active, created_at, updated_at
...
func (r *StaffRepository) List(ctx context.Context) ([]model.Staff, error) {
	query := `SELECT id, name, email, role, phone, service_incentive_rate, product_incentive_rate, base_salary, monthly_target, is_active, created_at, updated_at
